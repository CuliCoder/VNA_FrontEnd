...
export interface UserImportPreviewModalProps {
  open: boolean;
...
export default function UserImportPreviewModal({
  open,
  previewData,
  roles = [],
  onClose,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-5xl mx-4 max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-150">
        {/* Header */}
        <div className="bg-gradient-to-r from-blue-600 to-blue-500 px-5 py-4 flex items-center justify-between shrink-0">
          <h2 className="text-lg font-semibold text-white">Xem trước dữ liệu Import</h2>
          <button
            onClick={onClose}
            disabled={isLoading}
              ...
              <table className="w-full text-sm text-left">
                <thead className="bg-gray-50 border-b border-gray-200 text-gray-600 font-medium whitespace-nowrap">
                  <tr>
                    ...
                    <th className="px-4 py-3 w-40">Tài khoản</th>
                    <th className="px-4 py-3 w-48">Họ và tên</th>
                    <th className="px-4 py-3 w-48">Email</th>
                    <th className="px-4 py-3 w-32">Vai trò</th>
                    <th className="px-4 py-3 w-32">Chức danh</th>
                    <th className="px-4 py-3 w-32">Trạng thái HĐ</th>
                    <th className="px-4 py-3 min-w-[200px]">Chi tiết lỗi</th>
                  </tr>
                </thead>
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-gray-800">
                        {row.data?.isActive !== undefined 
                          ? (row.data.isActive ? "Hoạt động" : "Khóa") 
                          : "—"}
                      </td>
                      <td className="px-4 py-3">
