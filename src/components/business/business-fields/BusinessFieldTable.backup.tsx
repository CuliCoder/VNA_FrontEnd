...
export default function BusinessFieldTable({
  ...
  const filteredData = data.filter((item) => {
    ...
    const matchLevel = filterLevel === "all" || item.level === Number(filterLevel);
    const matchStatus = filterStatus === "all" || (filterStatus === "active" && item.status) || (filterStatus === "inactive" && !item.status);
    return matchCode && matchName && matchStatus && matchLevel;
  });
              ...
              paginatedData.map((item) => {
                ...
                return (
                  <tr key={item.id} className={`transition-colors hover:bg-gray-50 ${isSelected ? "bg-blue-50/60" : ""}`}>
                    ...
                    <td className="px-4 py-3 text-center align-middle">
                      <button className="text-gray-400 hover:text-gray-600" onClick={() => onEdit(item)}>
                        <Edit3 className="w-4 h-4" />
                      </button>
                    </td>
                    <td className="px-4 py-3 align-middle font-medium">{item.code}</td>
