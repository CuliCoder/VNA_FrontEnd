...
import { ChevronDown, ChevronRight, LogOut, Key, User as UserIcon, Menu } from "lucide-react";
                ...
                {menu.children ? (
                  <div>
                    ...
                    >
                      ...
                      {!collapsed && (
                        <>
                          <span className="flex-1 text-left leading-tight">{menu.title}</span>
                          {isOpen ? (
                    ...
                    {(isOpen || isGroupActive) && !collapsed && (
                      <ul className="bg-[#182f77]">
                        {menu.children
                          ...
                          .map((child) => {
                            const isActive =
                              child.url ? pathname === child.url || pathname.startsWith(child.url) : false;
                            return (
                              <li key={child.title}>
                                <Link
                                  href={child.url || "#"}
                  ...
                  >
                    ...
                    {!collapsed && <span className="leading-tight">{menu.title}</span>}
                  </Link>
                )}
              </li>
            );
        ...
        {isProfileOpen && (
          <div className="absolute bottom-full left-3 mb-2 w-52 bg-white text-gray-800 rounded-xl shadow-2xl py-2 text-sm z-50 ring-1 ring-gray-100">
            <div className="px-4 py-2.5 border-b border-gray-100">
              <p className="font-semibold text-gray-900 text-sm truncate">{user?.fullName || user?.username}</p>
              <p className="text-xs text-gray-400 truncate mt-0.5">{user?.role?.name}</p>
            </div>
            <Link
            ...
            <button className="w-full flex items-center gap-2.5 px-4 py-2.5 hover:bg-gray-50 transition-colors">
